...
export class VRView extends baseView {
  ...
  set rescaleLUT(bool) {
    ...
    if (this._rescaleLUT && this._PGwidget) {
      ...
      range = this._actor.getMapper().getInputData().getPointData().getScalars().getRange()
    }
  ...
  set rangeLUT([min, max]) {
    ...
    this._actor.getProperty().getRGBTransferFunction(0).setMappingRange(min, max)
  }
  ...
  set blurOnInteraction(toggle) {
    ...
    if (toggle) {
      ...
      interactor.onLeftButtonRelease(() => {
        ...
        if (this._pickingPlane) this._pickingPlane.setNormal(camera.getDirectionOfProjection())
        this._renderWindow.render()
      })
    } else {
      interactor.onLeftButtonPress(() => {
  ...
  addSurface({ buffer, color, label }) {
    if (this._surfaces.has(label)) {
      console.warn(`DTK: A surface with label ${label} is already present. I will ignore this.`)
      return
    }
   ...
   */
  _updateWidget() {
    const dataArray = this._actor.getMapper().getInputData().getPointData().getScalars()
    ...
    if (gaussians.length > 0) {
      ...
      this._PGwidget.addGaussian(this._wl, default_opacity, this._ww, default_bias, default_skew) // x, y, ampiezza, sbilanciamento, andamento
    }
  ...
  _setupInteractor() {
    ...
    const rotateManipulator = vtkMouseCameraTrackballRotateManipulator.newInstance({ button: 1 })
    ...
    this._renderWindow.getInteractor().onMouseWheel(() => this.resetMeasurementState())
    this._renderWindow.getInteractor().onRightButtonPress(() => this.resetMeasurementState())
  }
  ...
  _initPicker(state, mode) {
    ...
    this._leftButtonCb = this._renderWindow.getInteractor().onLeftButtonPress((callData) => {
      ...
      if (picker.getActors().length === 0) {
        ...
        if (this.VERBOSE) console.log(`No point picked, default: ${pickedPoint}`)
