...
import { getPlaneIntersection, getVolumeCenter, createVolumeActor } from './utils/utils'
...
export class MPRManager {
  ...
  set wwwl([ww, wl]) {
    ...
    this.volume.getProperty().getRGBTransferFunction(0).setMappingRange(lower, upper)
  ...
  onCrosshairPointSelected({ srcKey, worldPos }, externalState) {
    Object.keys(this.elements).forEach((key) => {
      if (key !== srcKey) {
        ...
        // ~ swerik
        const renderWindow = this.mprViews[key]._genericRenderWindow.getRenderWindow()
  ...
  onScrolled(state) {
    ...
    if (!Number.isNaN(newPoint) && !newPoint.some((coord) => Number.isNaN(coord))) {
