...
import { createVolumeActor, setCamera, setActorProperties, setupCropWidget } from './utils/utils'
      ...
      viewWidget.setDisplayCallback((coords) => {
        ...
        if (coords) {
          ...
          overlay.style.left = `${Math.round(
            (coords[0][0] / w) * window.innerWidth - overlaySize * 0.5 - overlayBorder
          )}px`
          overlay.style.top = `${Math.round(
            ((h - coords[0][1]) / h) * window.innerHeight - overlaySize * 0.5 - overlayBorder
          )}px`
        }
      })
