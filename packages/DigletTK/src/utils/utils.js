...
export function buildVtkVolume(header, data) {
  const dims = [header.volume.cols, header.volume.rows, header.volume.imageIds.length]
...
export function getRelativeRange(actor, absoluteRange) {
  const dataArray = actor.getMapper().getInputData().getPointData().getScalars()
...
export function setupPGwidget(PGwidgetElement) {
  ...
  let containerHeight = PGwidgetElement ? PGwidgetElement.offsetHeight - 5 : 100
  ...
  window.addEventListener('resize', (evt) => {
    PGwidget.setSize(PGwidgetElement.offsetWidth - 5, PGwidgetElement.offsetHeight - 5)
    PGwidget.render()
  })
