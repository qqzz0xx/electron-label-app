...
function InteractorStyleMPRSlice(publicAPI, model) {
  // Set our className
  ...
  publicAPI.getSlice = function () {
    ...
    const camera = renderer.getActiveCamera()
    const sliceNormal = publicAPI.getSliceNormal() // Get rotation matrix from normal to +X (since bounds is aligned to XYZ)
  ...
  publicAPI.setSlice = function (slice) {
    ...
    if (model.volumeMapper) {
      const range = publicAPI.getSliceRange()
      const bounds = model.volumeMapper.getBounds()
      const clampedSlice = clamp(slice, range[0], range[1])
      const center = [
  ...
  publicAPI.getSliceRange = function () {
    if (model.volumeMapper) {
      ...
      const bounds = model.volumeMapper.getBounds()
      const points = boundsToCorners(bounds) // Get rotation matrix from normal to +X (since bounds is aligned to XYZ)
  ...
  publicAPI.setSliceNormal = function (normal) {
    const renderer = model._interactor.getCurrentRenderer()
    ...
    if (model.volumeMapper) {
      const bounds = model.volumeMapper.getBounds() // diagonal will be used as "width" of camera scene
      ...
      const angle = 90 // distance from camera to focal point
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  InteractorStyleMPRSlice(publicAPI, model)
}
